  ...
  return (
    <main className="bg-background min-h-screen overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-8 md:px-16 h-16 flex items-center justify-center">
          <div className="flex items-center gap-8">
            ...
            <a
              className="text-xs tracking-[0.15em] uppercase border border-foreground text-foreground px-4 py-2 hover:bg-foreground hover:text-background transition-all duration-500 font-body font-medium"
              href="https://trustworthyailab.substack.com/"
              target="_blank"
      ...
      <section className="min-h-screen flex flex-col items-center justify-center text-center px-8 pt-16 relative">
        ...
        <FloatingImage src={star} alt="" className="absolute top-28 left-8 md:left-20 w-14 md:w-20" delay={0.8} />
        ...
        >
          <div className="relative w-72 mb-10 flex items-center justify-center">
            <img src={portrait} alt="Ariella Shulman" className="w-full h-auto" />
          </div>
      ...
      <section id="about" className="py-20 md:py-28 px-8 md:px-16 relative">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16">
          <motion.div
            ...
            custom={0}
          >
            <p className="text-sm tracking-[0.3em] uppercase text-foreground font-body font-medium">About</p>
          </motion.div>
          <motion.div
          ...
          >
            <div className="space-y-5 text-muted-foreground font-body text-base leading-relaxed">
              <p>
                by examining the deployment of ai in critical social infrastructure, i seek to understand how ai will
                change the moral and epistemic fabric of our society. my research spans philosophy, computer-human
                interaction, cognitive science, and policy.
              </p>
              <p>
                currently, i am studying for an{" "}
                <a
                  href="https://www.lcfi.ac.uk/education/mphil"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="underline hover:text-foreground transition-colors duration-500"
                >
                  mphil in ethics of ai, data &amp; algorithms
                </a>{" "}
                at the university of cambridge. i also conduct research in cambridge's{" "}
                <a
                  href="https://trace-lab.ai/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="underline hover:text-foreground transition-colors duration-500"
                >
                  trustworthy ai lab (trace)
                </a>{" "}
                and i am working on developing safety frameworks for ai tutors with{" "}
                <a
                  href="https://eedi-labs-67qwnm2lxu4blqi5.webflow.io/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="underline hover:text-foreground transition-colors duration-500"
                >
      ...
      <section id="projects" className="py-20 md:py-28 px-8 md:px-16">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16">
          <motion.div
            ...
            custom={0}
          >
            <p className="text-sm tracking-[0.3em] uppercase text-foreground font-body font-medium">Projects</p>
          </motion.div>
      ...
      <footer className="py-16 px-8 md:px-16 border-t border-border relative">
        <div className="max-w-6xl mx-auto flex-col text-center gap-4 flex items-center justify-center">
          <p className="text-xs text-muted-foreground tracking-wide">© {new Date().getFullYear()} Ariella Shulman</p>
        </div>
      </footer>
    </main>
  );
